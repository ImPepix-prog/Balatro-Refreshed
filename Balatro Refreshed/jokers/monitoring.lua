...
SMODS.Joker{ --Monitoring
    ...
    calculate = function(self, card, context)
        ...
        if context.end_of_round and context.game_over == false and context.main_eval  and not context.blueprint then
            if to_big((card.ability.extra.MonitoringMult or 0)) >= to_big(100) then
                return {
                    func = function()
                    ...
                    extra = {
                        func = function()
                            ...
                            G.E_MANAGER:add_event(Event({
                                func = function()
                                    local joker_card = SMODS.add_card({ set = 'Joker', key = 'j_[Refreshed]monitoringbestfriendedition' })
                                    if joker_card then
